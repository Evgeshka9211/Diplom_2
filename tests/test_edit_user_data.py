...
class TestChangingUserData:
    ...
    def test_edit_user_email_with_auth(self, create_user):
        ...
        with allure.step("Отправляем запрос на изменение email зарегистрированного пользователя"):
    ...
    def test_edit_user_password_with_auth(self, create_user):
        ...
        token = {'Authorization': create_user[3]}
        with allure.step("Отправляем запрос на изменение пароля зарегистрированного пользователя"):
            r = requests.patch(f"{Urls.MAIN_URL}{Urls.USER_URL}", headers=token, data=payload)
        with allure.step("Запрос отправлен, проверяем результат выполнения"):
    ...
    def test_edit_user_name_with_auth(self, create_user):
        ...
        token = {'Authorization': create_user[3]}
        with allure.step("Отправляем запрос на изменение имени зарегистрированного пользователя"):
            r = requests.patch(f"{Urls.MAIN_URL}{Urls.USER_URL}", headers=token, data=payload)
        with allure.step("Запрос отправлен, проверяем результат выполнения"):
    ...
    def test_edit_user_data_not_auth(self):
        with allure.step("Отправляем запрос на изменение данных пользователя без авторизации"):
